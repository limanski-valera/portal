(() => {
    ...
    function initSliders() {
        if (document.querySelector(".testimonials__slider")) new core(".testimonials__slider", {
            ...
            speed: 800,
            navigation: {
                prevEl: ".testimonials__slide-button_prev",
                nextEl: ".testimonials__slide-button_next"
            },
            breakpoints: {
                0: {
                    slidesPerView: 1,
    ...
    function animateActivities() {
        if (document.querySelector(".activities__item")) {
            const items = document.querySelectorAll(".activities__item");
            let itemIndex = 0;
            const toggleActivities = items => {
                setInterval((() => {
                    itemIndex === items.length ? itemIndex = -1 : 0;
                    items.forEach(((item, index) => {
                        if (index === itemIndex && item.classList.contains("_active")) item.classList.remove("_active");
                        if (index === itemIndex + 1) item.classList.add("_active");
                    }));
                    itemIndex++;
                }), 2e3);
            };
            toggleActivities(items);
        }
    }
    document.addEventListener("load", wondowLoad());
    function wondowLoad() {
        animateActivities();
    }
    window["FLS"] = true;
